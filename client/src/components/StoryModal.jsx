...
const StoryModal = ({setShowModal, fetchStories}) => {
    ...
    const handleCreateStory = async () => {
      ...
      try {
        const { data } = await api.post('/api/story/create', formData, {headers: 
        ...
        style={{backgroundColor: background}}>
          ...
          {mode === 'text' && (
            <textarea className='bg-transparent text-white w-full h-full p-6 text-lg resize-none
             focus:outline-none' placeholder="What's on your mind?" onChange={(e)=>setText(e.target.value)}
             value={text}/>
          )}
          {
          ...
          <div className='flex gap-2 mt-4'>
            ...
            <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded 
              cursor-pointer ${mode === 'media' ? "bg-white text-black" : "bg-zinc-800"}`}>
            ...
            hover:from-indigo-600 hover:to-purple-700 active:scale-95 tranistion cursor-pointer'>
